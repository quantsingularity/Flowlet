import { Building, CheckCircle, Copy, Download, QrCode, Share2 } from "lucide-react";
  ...
  return (
    <div className="space-y-1.5">
      ...
      <div className="flex items-center gap-2">
        <Input value={value} readOnly className="font-mono text-sm bg-muted/50 border-0" />
        <Button
          variant="ghost"
          size="sm"
  ...
  return (
    <div className="max-w-lg mx-auto space-y-5 animate-fade-in">
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-1">Share your account details to receive funds</p>
      </div>
      ...
      <Card className="overflow-hidden border-0 bg-sidebar text-sidebar-foreground">
        <CardContent className="p-6">
          <div className="flex items-start justify-between mb-4">
            <div>
              <p className="text-sidebar-foreground/60 text-xs uppercase tracking-widest">Account Holder</p>
              <p className="text-xl font-bold mt-1">{accountDetails.accountName}</p>
            </div>
          ...
          <div className="space-y-1">
            <p className="text-sidebar-foreground/60 text-xs">{accountDetails.bankName}</p>
            <p className="font-mono text-lg tracking-wider">{accountDetails.accountNumber}</p>
          </div>
      ...
      <Tabs defaultValue="details" className="space-y-4">
        <TabsList className="h-9">
          <TabsTrigger value="details" className="text-xs">Account Details</TabsTrigger>
          <TabsTrigger value="qr" className="text-xs">QR Code</TabsTrigger>
          <TabsTrigger value="request" className="text-xs">Request Amount</TabsTrigger>
        </TabsList>
          ...
          <Card>
            <CardHeader className="pb-4">
              <div className="flex items-center justify-between">
                ...
                <Button variant="ghost" size="sm" className="text-xs gap-1.5 h-7" onClick={copyAll}>
            ...
            <CardContent className="space-y-4">
              <CopyField label="Account Name" value={accountDetails.accountName} />
              <CopyField label="Account Number" value={accountDetails.accountNumber} />
              <CopyField label="Routing Number (ACH / ABA)" value={accountDetails.routingNumber} />
              <CopyField label="SWIFT / BIC Code" value={accountDetails.swiftCode} />
              ...
              <div className="rounded-xl bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-900 p-3">
                <p className="text-xs text-blue-700 dark:text-blue-400">
                  <span className="font-semibold">Note:</span> International wires typically take 1–3 business days. Domestic ACH transfers settle the next business day.
                </p>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex gap-3">
                <Button className="flex-1 gap-1.5" disabled={!requestAmount || Number(requestAmount) <= 0}>
                  <Share2 className="h-4 w-4" />
                  Generate Request Link
                </Button>
                <Button variant="outline" className="gap-1.5" disabled={!requestAmount || Number(requestAmount) <= 0}>
